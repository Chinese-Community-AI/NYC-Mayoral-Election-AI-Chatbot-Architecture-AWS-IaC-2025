...
const jwtAuthLink = setContext((_, { headers }) => {
  ...
  console.log("Auth token status:", {
    isLoggedIn: !!token,
    tokenExpiry: token
      ? new Date(authService.getTokenExpiry()).toISOString()
      : "none",
  ...
  console.log("Request headers:", {
    ...
    authHeaderLength: authHeaders.headers.authorization
      ? authHeaders.headers.authorization.length
      : 0,
    otherHeaders: Object.keys(authHeaders.headers).filter(
  ...
  auth: {
    ...
    token: () => {
      ...
      console.log(
        ...
        token ? "Bearer " + token : "No token available"
      );
...
const errorLink = onError(({ graphQLErrors, networkError }) => {
  ...
  if (networkError) {
    ...
    if (
      ...
      console.error(
        "WebSocket authentication error. Check your token format and validity."
      );
        ...
        fields: {
          getMessages: {
            ...
            merge(existing = [], incoming, { args }) {
              console.log(
                "Merging messages for conversation:",
                args?.conversationId
              );
              ...
              if (!args || !args.conversationId) {
                console.log(
                  "No conversation ID provided, returning incoming data"
                );
                return incoming;
              }
              ...
              existing.forEach((msg) => {
                ...
                if (!msg || !msg.id) {
                  console.log(
                    "Skipping undefined message or message without ID in client merge"
                  );
                  return;
                }
              ...
              incoming.forEach((msg) => {
                ...
                if (!msg.id) {
                  console.log(
                    "Message without ID detected in merge function:",
                    msg
                  );
                ...
                if (msg.conversationId !== args.conversationId) {
                  console.log(
                    "Fixing message with incorrect conversationId:",
                    msg.id
                  );
                ...
                if (
                  ...
                  for (let i = -2; i <= 2; i++) {
                    ...
                    if (tempUserMessageMap.has(mapKey)) {
                      ...
                      if (tempIndex >= 0) {
                        console.log(
                          "Removing temporary message from merged array:",
                          tempId
                        );
                ...
                if (!existingMap.has(msg.id)) {
                  ...
                  msg.isComplete &&
                  !existingMap.get(msg.id).isComplete
                ) {
              ...
              console.log("Merged and sorted messages:", sorted.length, {
                byRole: {
                  ...
                  assistant: sorted.filter((msg) => msg.role === "assistant")
                    .length,
                },
                temporaryRemaining: sorted.filter((msg) =>
                  msg.id.startsWith("temp-user-")
                ).length,
                placeholders: sorted.filter((msg) =>
                  msg.id.startsWith("placeholder-")
                ).length,
              });
